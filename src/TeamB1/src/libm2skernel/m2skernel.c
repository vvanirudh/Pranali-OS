...
struct interrupt_t* first_interrupt = NULL;
struct interrupt_t* last_interrupt = NULL;
struct interrupt_t* least_interrupt = NULL;

static uint64_t least_instruction = 0;




void add_interrupt(struct interrupt_t* curr_interrupt){

    if(first_interrupt == NULL && last_interrupt == NULL){
        first_interrupt = curr_interrupt;
        last_interrupt = curr_interrupt;
        least_interrupt = curr_interrupt;
        least_instruction = curr_interrupt->ito;

        curr_interrupt->prev_interrupt = NULL;
        curr_interrupt->next_interrupt = NULL;
    }
    else{
        last_interrupt->next_interrupt = curr_interrupt;
        curr_interrupt->prev_interrupt = last_interrupt;
        curr_interrupt->next_interrupt = NULL;
        last_interrupt = curr_interrupt;

        if(curr_interrupt->ito < least_instruction){
            least_instruction = curr_interrupt->ito;
            least_interrupt = curr_interrupt;
        }
...
uint64_t get_least_interrupt_time()
{
    return least_instruction;
}


struct interrupt_t* get_least_interrupt()
{
    return least_interrupt;
}

void delete_interrupt(struct interrupt_t* curr_interrupt){
    if(curr_interrupt == first_interrupt && curr_interrupt==last_interrupt){
        first_interrupt = NULL;
        last_interrupt = NULL;
        free(curr_interrupt->details);
        free(curr_interrupt);
    }
    else if(curr_interrupt == first_interrupt){
        first_interrupt = curr_interrupt->next_interrupt;
        first_interrupt->prev_interrupt = NULL;
        free(curr_interrupt->details);
        free(curr_interrupt);
    }
    else if(curr_interrupt == last_interrupt){
        last_interrupt = curr_interrupt->prev_interrupt;
        last_interrupt->next_interrupt = NULL;
        free(curr_interrupt->details);
        free(curr_interrupt);
    }
    else{
        curr_interrupt->prev_interrupt->next_interrupt = curr_interrupt->next_interrupt;
        curr_interrupt->next_interrupt->prev_interrupt = curr_interrupt->prev_interrupt;
        free(curr_interrupt->details);
        free(curr_interrupt);       
    }

    struct interrupt_t* temp;
    if(first_interrupt == NULL && last_interrupt == NULL){
        least_interrupt = NULL;
        least_instruction = 0;
    }
    else{
        least_interrupt = first_interrupt;
        least_instruction = first_interrupt->ito; 
    }

    //resetting the least interrupt after deletion
    for(temp = first_interrupt;temp!=NULL;temp=temp->next_interrupt){
        if(temp->ito < least_instruction){
            least_instruction = temp->ito;
            least_interrupt = temp;
        }
    }
}
...
// extern uint64_t no_instructions;
// extern struct interrupt_t;
// extern struct interrupt_details_t;
...
void dk_init(void)
{
	dk = calloc(1, sizeof(struct disk_t));
	dk->heads = heads;
	dk->tracks = tracks;
	dk->sectors = sectors;
	dk->currentBlock = 0;
...
int dk_get_head(void)
{
	return dk->currentBlock;
}

/** IO TIME FUNCTION **/
int dk_get_time(int block)
{
	return abs(block - dk->currentBlock)*0.1;
}

void dk_update_head(int block)
{
	dk->currentBlock = block;
}
