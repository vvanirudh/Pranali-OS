...
int handle_guest_syscalls() {
    ...
    switch (syscode) {
        ...
        case syscall_code_io_disk:
        {
            int op = isa_regs->ebx; // write - 0
            uint32_t address = (uint32_t)isa_regs->edx;
            int no_of_blocks = isa_regs->esi;
            int block_location = isa_regs->edi;
            int no_of_bytes = isa_regs->ecx;
            int read_write_pos = 512*no_of_blocks + block_location;
    
            if (op == 0){
        
                int wfile = open("Sim_disk",O_WRONLY);
                if(wfile < 0) 
                fatal("Unable to locate the memory area on the disk");
                ...
                // ctx_set_status(isa_ctx,  ctx_blocked);
                    ...
                    int rfile = open("Sim_disk",O_RDONLY);
                    if(rfile < 0) 
                      fatal("Unable to locate the memory area on the disk");
                  void* buffer = calloc(1,no_of_bytes);
            ...
            struct interrupt_t * io_int;
            io_int = calloc(1, sizeof(struct interrupt_t));
            ...
            io_int->type = op;
            struct interrupt_details_t * io_int_details;
            io_int_details = calloc(1, sizeof(struct interrupt_details_t));
            io_int_details->pid = get_pid();
            io_int_details->p_ctx = isa_ctx;
            io_int->details = io_int_details;
            io_int->next_interrupt = NULL;
            io_int->prev_interrupt = NULL;
